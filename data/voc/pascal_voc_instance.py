...
class VOC12(data.Dataset):
    ...
    def __init__(self, root=Path.db_root_dir('VOCDetection'), split='instance', transform=None, download=True, ignore_classes=[]):
        # Set paths
        self.root = root
        valid_splits = ['trainaug', 'train', 'val', 'instance'] # instance should be defined
        assert(split in valid_splits)
        self.split = split

        _instance_dir = os.path.join(self.root, 'SegmentationObject')
        ...
        for ii, line in enumerate(lines):
            ...
            _instance_map = os.path.join(_instance_dir, line + '.png')
            assert os.path.isfile(_instance_map)
            self.instances.append(_instance_map)

        assert(len(self.imgs) == len(self.semsegs)  ==  len(self.instances))
    ...
    def __getitem__(self, index):
        ...
        _instance = self._load_instance(index)
        if _semseg.shape != _img.shape[:2]:
            _semseg = cv2.resize(_semseg, _img.shape[:2][::-1], interpolation=cv2.INTER_NEAREST)
        if _instance.shape != _img.shape[:2]:
            _instance.resize(_instance.shape[:2][::-1], interpolation=cv2.INTER_NEAREST)
        sample['semseg'] = _semseg

        sample['instance'] = _instance
    ...
    def _load_instance(self, index):
        _instance = np.array(Image.open(self.instances[index]))

        for ignore_class in self.ignore_classes:
            _instance[_instance == ignore_class] = 255
        return _instance
