...
def main():
    ...
    if os.path.exists(p['pretrained']):
        print(colored('Loading pretrained model from {}'.format(p['pretrained']), 'blue'))
        state_dict = torch.load(p['pretrained'], map_location='cpu')
        ...
        print(colored('Pretrained mode does not exist at {}'.format(p['pretrained']), 'blue'))
        sys.exit(1)
    
    # Freeze all layers except final 1 by 1 convolutional layer
    for name, param in model.named_parameters():
        if name not in ['head.4.weight', 'head.4.bias']:
            param.requires_grad = False
    model.head[4].weight.data.normal_(mean=0.0, std=0.01)
    model.head[4].bias.data.zero_()
